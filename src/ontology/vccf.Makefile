...
$(PATTERNDIR)/data/default/vessel.tsv $(TEMPLATEDIR)/vessel_relation.tsv: $(TMPDIR)/vessel.csv
	python ../scripts/etl/vessel_ETL.py --input $< --pattern $@ --template $(TEMPLATEDIR)/vessel_relation.tsv
...
$(TEMPLATEDIR)/vessel_organ_crosswalk.tsv: $(TMPDIR)/vessel_organ_crosswalk.csv
	python ../scripts/etl/vessel_organ_crosswalk_ETL.py --input $< --template $@ --vessels $(PATTERNDIR)/data/default/vessel.tsv
